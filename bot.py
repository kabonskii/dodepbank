...
def start(message):
    ...
    bot.send_message(message.chat.id, f"Ваш ID: {message.from_user.id}")  # проверка ID
...
def save_new_debt(message):
    ...
    bot.send_message(message.chat.id, f"Долг пользователя {user_id} обновлён до {new_debt}₽")
